...
class Harga_model extends CI_Model
{
    ...
    {
        $this->db->select("tbl_harga.id_harga, tbl_kategori.nama_kategori, tbl_barang.kode_barang, tbl_harga.stok_toko, tbl_barang.nama_barang, tbl_toko.nama_toko, tbl_barang.barcode_barang, (SELECT COALESCE(SUM(tbl_barang_masuk.jml_masuk), 0) FROM tbl_barang_masuk WHERE tbl_barang_masuk.harga_id = tbl_harga.id_harga) as jml_masuk, (SELECT COALESCE(SUM(tbl_barang_keluar.jml_keluar), 0) FROM tbl_barang_keluar WHERE tbl_barang_keluar.harga_id = tbl_harga.id_harga) as jml_keluar");
        $this->db->from("tbl_harga");
        $this->db->join("tbl_barang", "tbl_harga.barang_id = tbl_barang.id_barang", "inner");
