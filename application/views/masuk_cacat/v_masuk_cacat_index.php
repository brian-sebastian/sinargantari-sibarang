<div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="col-xl">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <button class="btn btn-primary btn-sm" id="add_idtoko" type="button">
    ...
    <div class="modal fade" id="lihatMasukCacat_Modal" tabindex="-1" aria-modal="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel3">Detail Barang Masuk Cacat</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="lihatMasukCacat">
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(function() {
            initializeTable("server-side", {
                "ajax": {
                    url: "<?= site_url('barang_cacat/masuk_cacat/ajax') ?>",
                    type: "post",
                    data: function(data) {
        ...
        $('#add_idtoko').click(function(event) {
            ...
            let url = '<?= base_url('barang_cacat/masuk_cacat/tambah') ?>?toko_id=' + encryptTokoId;
            window.open(url, '_blank');
        });

        function lihat_masuk_cacat(id) {
            //console.log(id)
            $('#lihatMasukCacat').empty()
            $.get('<?= base_url() ?>barang_cacat/masuk_cacat/lihat_masukcacat_model/' + btoa(id), function(data_masukcacat) {
                $('#lihatMasukCacat').html(data_masukcacat)
                $('#lihatMasukCacat_Modal').modal('show')
            })
        }
    </script>